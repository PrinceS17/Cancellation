...
)
{
	...
	int num = 0;
	float amplitude = 0.5;
	/*vector <complex<float> > symbol(4);				     // QPSK symbol
	symbol[0] = amplitude*(1,1),symbol[1] = amplitude*(1,-1),symbol[2] = amplitude*(-1,1),symbol[3] = amplitude*(-1,-1);*/
...
if(!ifreal)
{
qpsk_data.open("qpsk_tx",ios::in | ios::binary);
qpsk_data.seekg(0,qpsk_data.end);
sam_num = qpsk_data.tellg()/8;
cout<<"sam_num = "<<sam_num<<endl;
qpsk_data.seekg(0,qpsk_data.beg);
for(int i = 0; i < sam_num; i++)
{
	qpsk_data.read((char*)&t[0],sizeof(float));
	qpsk_data.read((char*)&t[1],sizeof(float));
	buff[i] = (t[0],t[1]);
...
else
{
	qpsk_data.open("qpsk_tx",ios::in | ios::binary);
	qpsk_data.seekg(0,qpsk_data.end);
	sam_num = qpsk_data.tellg()/4;
	cout<<"sam_num = "<<sam_num<<endl;
	qpsk_data.seekg(0,qpsk_data.beg);
	float temp;
for(int i = 0; i < sam_num; i++)
{
	qpsk_data.read((char*)&temp,sizeof(float));
	buff[i] = complex<float>(temp,0);		// buff[i] is still (0,0)??!!
	cout<<buff[i]<<endl;
}
qpsk_data.close();
	...
	while(not stop_signal_called)	
	{
		...
		{
			// buff[n] = n%3;
			buff[n] = symbol[n%4];						  // repeat all symbols
		}*/
...
int UHD_SAFE_MAIN(int argc,char *argv[]){
	...
	string type = "float";
	string file = "qpsk_rx_file";

	double rate,rx_rate,tx_rate,freq,wave_freq,gain,bw,ampl;
	rate = 3e6;
	// wave_freq = 100e3;		// no need in qpsk
	freq = 915e6;
	gain = 25;
	bw = 1e6;
	rx_rate = rate;
...
int spb = 6000;		// adjust to the size of qpsk input
