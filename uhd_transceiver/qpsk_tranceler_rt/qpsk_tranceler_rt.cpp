	...
	)
{
	...
	while(not stop_signal_called)
	{
		...
		if(delay_const < 0)						// if rx1 read but not synchronized
		{
			...
			delay_const = 137280;					// manually; if too large, y is 0s without data -> peaks lack
...
*/
	int rx_begin_length = 0*spb;		// for sine and 0s, 10 buff in all
