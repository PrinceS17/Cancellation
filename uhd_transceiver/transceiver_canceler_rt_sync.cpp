...
// x2A: contains nonlinearity, dim = 1 means linear by default
	...
	)
{
	...
	bool ifinit = global_num[0]*global_num[1];			// seems that ifinit can be merged into if_syned
	...
	if(!ifinit)  { mtx.unlock(); continue;}
	else
	{
	int min_num = min(global_num[0],global_num[1]);
	if(!if_synced && min(global_num[0],global_num[1]) > 5)		// suppose 5 buffer is the biggest delay
	{
		rx1 = VectorXf::Zero(global_rx.size());
		for(int i = 0; i < rx1.size(); i ++)
			rx1[i] = global_rx[i].real();
		if_synced = 1;
	}
	...
	{
		x(i) = global_tx[can_pos + i].real();
		y(i) = global_rx[can_pos + i + delay_const].real();	// detail of y_clean's length should be notice
	}
	...
	if(delay_const < 0)						// if haven't synchronized
	{
...
int UHD_SAFE_MAIN(int argc,char *argv[]){
	...
	const wave_table_class preamble_table(pre_wt,ampl);
	...
	int num_buff = 100;				// test which is fine enough
...
cout<<"-- after assign..."<<endl;
	...
	threads.create_thread(boost::bind(&dg_sic, preamble_vf, preamble, estimator_length, preamble_length, pilot_length, signal_length, rate));
